...
- ② -B FV 内テキスト 5要素の text-shadow を全削除（`.hero__title` / `.hero__sub` / `.hero__sub-en` / `.hero__label` / `.hero__scroll`）
...
### ② -B ／ FV 内テキスト 5要素の text-shadow を全削除

写真本来の質感（空の透明感）を活かすため、FV（hero セクション）内のテキスト 5要素のシャドウを **すべて削除** します。新しい空系オーバーレイのグラデーション（下部 0.55〜0.85 alpha）が十分なコントラストを与えるため、白文字の可読性は保たれます。

**対象（5要素・該当行は本番 main.css 内）**

| クラス | 役割 | 元の text-shadow | 変更後 |
|---|---|---|---|
| `.hero__title` | 大見出し「あなたが、長く続けられる場所を。」 | 2層 (0.45 / 0.55) | `none` |
| `.hero__sub` | サブコピー「美容学生・高校生…」 | 3層 (0.85 / 0.9 / 0.75) | `none` |
| `.hero__sub-en` | 「iBROなら、美容はもっと面白い。®」 | 2層 (0.6 / 0.7) | `none` |
| `.hero__label` | 左上「RECRUIT」 | 1層 (0.55) | `none` |
| `.hero__scroll` | 下部「SCROLL」 | 1層 (0.5) | `none` |

**まとめ置換例**

該当の各セレクタ内の `text-shadow:` を **すべて** 以下に置換するだけです。

```css
text-shadow: none;
```

複数行に分かれている場合（`.hero__sub` など）はその複数行をまとめて `text-shadow: none;` の1行に置換してください。
...
6. **変更② -B** `.hero__title` / `.hero__sub` / `.hero__sub-en` / `.hero__label` / `.hero__scroll` の **5要素すべての `text-shadow:` 行** を `text-shadow: none;` に置換
7. 「ファイルを更新」

### 方法B：FTP / SFTP 経由

`wp-content/themes/LIM_responsive001/assets/css/main.css` を取得して、ローカルで上記の編集を行い再アップロード。
...
- 変更② -B：FV 5要素の元の text-shadow に戻す（各クラスの旧値は上表参照）
